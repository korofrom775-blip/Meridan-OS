...
(function(global){
  ...
  function renderTray(){
    ...
    document.getElementById('tray-clock').addEventListener('click', toggleCalendarPopover);
    tickClock();
  }

  /* ---------------- tray calendar popover ---------------- */
  var calMonth = null; /* Date set to the 1st of the viewed month, or null (defaults to today) */
  function toggleCalendarPopover(){
    var existing = document.getElementById('cal-popover');
    if(existing){ existing.remove(); document.removeEventListener('mousedown', calOutside, true); return; }
    calMonth = new Date(); calMonth.setDate(1);
    renderCalendarPopover();
    setTimeout(function(){ document.addEventListener('mousedown', calOutside, true); }, 0);
  }
  function calOutside(e){
    var el = document.getElementById('cal-popover');
    var clock = document.getElementById('tray-clock');
    if(el && !el.contains(e.target) && !(clock && clock.contains(e.target))){
      el.remove();
      document.removeEventListener('mousedown', calOutside, true);
    }
  }
  function renderCalendarPopover(){
    var old = document.getElementById('cal-popover');
    if(old) old.remove();
    var panel = document.createElement('div');
    panel.id = 'cal-popover';
    ...
    panel.innerHTML =
      '<div class="cal-head">'+
        '<button id="cal-prev" title="Previous month">'+svg('chevronLeft')+'</button>'+
        '<h3>'+monthNames[month]+' '+year+'</h3>'+
        '<button id="cal-next" title="Next month">'+svg('chevronRight')+'</button>'+
      '</div>'+
      '<div class="cal-dow">'+['S','M','T','W','T','F','S'].map(function(x){ return '<div>'+x+'</div>'; }).join('')+'</div>'+
      '<div class="cal-grid">'+cells+'</div>'+
      '<div class="cal-foot"><button id="cal-today">Today</button></div>';
    document.getElementById('app').appendChild(panel);
    panel.querySelector('#cal-prev').addEventListener('click', function(){ calMonth.setMonth(calMonth.getMonth()-1); renderCalendarPopover(); });
    panel.querySelector('#cal-next').addEventListener('click', function(){ calMonth.setMonth(calMonth.getMonth()+1); renderCalendarPopover(); });
    panel.querySelector('#cal-today').addEventListener('click', function(){ calMonth = new Date(); calMonth.setDate(1); renderCalendarPopover(); });
  }
