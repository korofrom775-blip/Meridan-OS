...
(function(global){
  ...
  /* ---------------- shell service worker ----------------
     Caches the OS shell's own assets for instant repeat loads and a bit
     of offline resilience. Only registers over http(s) — service workers
     aren't available at all when the app is opened directly via file://,
     so this quietly no-ops there instead of logging noisy errors. */
  function registerShellServiceWorker(){
    if(!('serviceWorker' in navigator)) return;
    if(location.protocol !== 'http:' && location.protocol !== 'https:') return;
    navigator.serviceWorker.register('sw.js').catch(function(err){
      console.warn('[meridian] shell service worker registration failed:', err.message);
    });
  }
