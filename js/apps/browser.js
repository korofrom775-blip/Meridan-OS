...
(function(global){
  ...
  // Cloudflare Workers proxy — handles all sites, no local server needed.
  // Meridian Worker proxy (Cloudflare) — handles all sites, no local server needed.
  var PROXY_BASE = 'https://jolly-forest-5a13.korofrom775.workers.dev/?url=';
  function viaProxy(url){ return PROXY_BASE ? PROXY_BASE + encodeURIComponent(url) : url; }
  ...
  function normalize(raw, engine){
    ...
    var eng = SEARCH_ENGINES[engine] || SEARCH_ENGINES.brave;
  ...
  function buildBrowser(ctx){
    ...
    function newTab(url, opts){
      opts = opts || {};
      var t = { id: uid('tab'), title: opts.title || 'New Tab', url: url||HOME, stack:[url||HOME], idx:0, zoom:1, pinned: !!opts.pinned, private: !!opts.private };
      tabs.push(t);
      if(!opts.background){
        activeId = t.id;
        renderTabs();
    ...
    // Persists the open (non-private) tabs so the Browser can pick up where
    // it left off next time it's opened — like a real browser's session
    // restore. Debounced since url/title updates can fire in quick bursts.
    var saveSessionDebounced = OS.Util.debounce(function(){
      var nonPrivate = tabs.filter(function(t){ return !t.private; });
      var snapshot = nonPrivate.map(function(t){ return { url:t.url, pinned:t.pinned, title:t.title }; });
      var activeIndex = nonPrivate.findIndex(function(t){ return t.id===activeId; });
      OS.Storage.setBrowserSession(userId, { tabs: snapshot, activeIndex: activeIndex>=0 ? activeIndex : 0 });
    }, 400);
    function restoreOrStart(){
      if(ctx.data && ctx.data.url){ newTab(ctx.data.url); return; }
      var session = OS.Storage.getBrowserSession(userId);
      if(session && session.tabs && session.tabs.length){
        session.tabs.forEach(function(s){ newTab(s.url, { pinned:s.pinned, title:s.title, background:true }); });
        var idx = (session.activeIndex!=null && tabs[session.activeIndex]) ? session.activeIndex : 0;
        activeId = tabs[idx].id;
        renderTabs();
        renderActive();
      } else {
        newTab(bset.homepage);
      }
    }
      ...
      els.tabsWrap.querySelectorAll('.br-tab').forEach(function(node){
        ...
        node.addEventListener('contextmenu', function(e){
          ...
          OS.UI.showContextMenu(e.clientX, e.clientY, [
            ...
            { label: tb.pinned ? 'Unpin tab' : 'Pin tab', icon:'pin', action:function(){ tb.pinned = !tb.pinned; renderTabs(); saveSessionDebounced(); } },
            { divider:true },
    ...
    els.frame.addEventListener('load', finishLoading);

    // The proxy's injected shim posts the real URL and <title> of whatever
    // loaded inside the iframe (including clicks/redirects/SPA navigation
    // and title changes that happen entirely inside the proxied page).
    // Keep the address bar, tab title and history in sync with both.
    window.addEventListener('message', function(e){
      if(!e.data || e.data.type !== 'meridian-nav' || !e.data.url) return;
      var t = tabForFrameMessage(); if(!t) return;
      var urlChanged = e.data.url !== t.url;
      var realTitle = (e.data.title||'').trim();
      if(urlChanged){
        t.url = e.data.url;
        t.stack[t.idx] = t.url; // in-page navigation replaces the current entry rather than pushing a new one
      }
      t.title = realTitle || titleFor(t.url);
      renderTabs();
      saveSessionDebounced();
      if(t.id === activeId){ els.addr.value = t.url; updateStar(t.url); }
      if(urlChanged && !t.private) recordHistory(t.url, t.title);
      else if(!t.private) updateLastHistoryTitle(t.url, t.title);
    });    // The message's source frame is always the browser's own iframe (it has
    // no nested browser instances), so the active tab is the right target.
    function tabForFrameMessage(){ return activeTab(); }
    function updateLastHistoryTitle(url, title){
      var list = OS.Storage.getHistory(userId);
      if(list.length && list[0].url === url && list[0].title !== title){
        list[0].title = title;
        OS.Storage.setHistory(userId, list);
      }
    }
    ...
    function copyToClipboard(text){
      if(!text) return;
      var done = function(){ OS.UI.toast({ title:'Copied', body:text, icon:'copy' }); };
    ...
    ctx.bodyEl.querySelector('.br-navbar').addEventListener('click', function(e){
      ...
      if(nav === 'back' && t.idx>0){ t.idx--; t.url = t.stack[t.idx]; t.title=titleFor(t.url); renderTabs(); render(t); saveSessionDebounced(); }
      else if(nav === 'fwd' && t.idx<t.stack.length-1){ t.idx++; t.url = t.stack[t.idx]; t.title=titleFor(t.url); renderTabs(); render(t); saveSessionDebounced(); }
    ...
    function renderSuggestions(query){
      ...
      if(query.trim()){
        var eng = SEARCH_ENGINES[bset.searchEngine] || SEARCH_ENGINES.brave;
    ...
    ctx.bodyEl.addEventListener('keydown', function(e){
      ...
      else if(k==='d'){ e.preventDefault(); toggleBookmark(); }
    ...
    ctx.bodyEl.querySelector('#br-menu').addEventListener('click', function(e){
      ...
      OS.UI.showContextMenu(e.clientX, e.clientY, [
        ...
        { divider:true },
        { label:'Copy link address', icon:'copy', disabled: !t || !t.url, action:function(){ copyToClipboard(t.url); } },
