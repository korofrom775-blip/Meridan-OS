...
(function(global){
  ...
  // Set this once your proxy is deployed and reachable over HTTPS (see proxy-deploy/README.md).
  // Leave as '' to load sites directly in the iframe (old behavior, no popup protection).
  var PROXY_BASE = 'http://localhost:8080/proxy?url='; // only works if the proxy is running on this same machine
  ...
  function normalize(raw, engine){
    ...
    var eng = SEARCH_ENGINES[engine] || SEARCH_ENGINES.duckduckgo;
  ...
  function buildBrowser(ctx){
    ...
    function render(tab){
      ...
      if(m){
        ...
        hideInternalPages();
        startLoading();
        els.frame.src = viaProxy(tab.url);
        applyZoom(tab);
      }
    }
    ...
    function reloadTab(t){
      ...
      else { var u = t.url; els.frame.src='about:blank'; startLoading(); setTimeout(function(){ els.frame.src=viaProxy(u); applyZoom(t); }, 30); }
    }
    ...
    function renderSuggestions(query){
      ...
      if(query.trim()){
        var eng = SEARCH_ENGINES[bset.searchEngine] || SEARCH_ENGINES.duckduckgo;
