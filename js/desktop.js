...
(function(global){
  ...
  function addIcon(appId){
    if(!layout || layout.icons.some(function(i){ return i.id===appId; })) return;
    var spot = freeSpot();
    layout.icons.push({ id:appId, x:spot.x, y:spot.y });
    persist(); draw();
  }
  function removeIcon(appId){
    if(!layout) return;
    layout.icons = layout.icons.filter(function(i){ return i.id !== appId; });
    persist(); draw();
  }
  ...
  OS.Desktop = { render:render, pinnedAppIds:pinnedAppIds, togglePin:togglePin, isPinned:isPinned, addIcon:addIcon, removeIcon:removeIcon };
