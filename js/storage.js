...
(function(global){
  ...
  function defaultDesktop(){
    return { icons:[
      ...
      { id:'tasks',    x:24, y:342 },
      { id:'paint',    x:24, y:448 },
      { id:'terminal', x:24, y:554 },
      { id:'settings', x:24, y:660 }
  ...
  function defaultBrowserSettings(){
    return { searchEngine:'brave', homepage:'meridian://home', clearHistoryOnExit:false };
  }
  ...
  global.OS.Storage = {
    ...
    getBrowserSession: function(userId){ return read(userKey(userId,'browserSession'), null); },
    setBrowserSession: function(userId, val){ return write(userKey(userId,'browserSession'), val); },

    wipeUserData: function(userId){
      ['settings','desktop','fs','bookmarks','history','downloads','browserSettings','notifications','tasks','browserSession'].forEach(function(n){
