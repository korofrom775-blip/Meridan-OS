...
/* =================================================================
   ...
   Then open http://localhost:3001 — this server hosts both the app
   itself and the proxy endpoint it talks to, so there's nothing else
   to run and no separate frontend host to configure. Deploying this
   folder to any Node host (Render, Railway, Fly.io, etc.) works the
   same way: one service, one URL, works for everyone who visits it.
   ================================================================= */
...
// Serve the app itself (index.html, css/, js/) from this same server, so
// one deploy gives you both the site and the proxy — no separate frontend
// hosting, no cross-origin config, no localhost URLs baked into the code.
